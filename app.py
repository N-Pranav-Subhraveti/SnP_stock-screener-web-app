...
def check_uptrend_filter(df: pd.DataFrame, strategy_name: str, filter_params: Dict) -> Tuple[bool, Dict[str, Any]]:
    ...
    if strategy_name == 'rsi':
        conditions = {
            'price_above_ema20': price > ema_20 * (filter_params.get('price_vs_ema20', 98) / 100),
            'price_above_ema50': price > ema_50 * (filter_params.get('price_vs_ema50', 97) / 100),
            'rsi_reasonable': filter_params.get('rsi_min', 30) <= rsi <= filter_params.get('rsi_max', 75),
            'volume_adequate': volume_ratio > filter_params.get('volume_ratio_min', 0.7)
        }
        required_passes = filter_params.get('required_conditions', 3)
    elif strategy_name == 'ma_crossover':
        conditions = {
            'price_above_ema20': price > ema_20 * price_vs_ema20_threshold,
            'price_above_ema50': price > ema_50 * price_vs_ema50_threshold,
        ...
        conditions = {
            ...
            'rsi_not_extreme': rsi > filter_params.get('rsi_min', 25),
            'volume_adequate': volume_ratio > filter_params.get('volume_ratio_min', 0.5)
        }
